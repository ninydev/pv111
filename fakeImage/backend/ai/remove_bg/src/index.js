...
const sendMix = require('./helpers/send_mix');

const removeBg = async (fakeImage) => {

    // Операция по запросу к AI тут
    console.log(fakeImage);

    fakeImage.photo_url_no_bg = 'Ссылка на файл без фона';
    fakeImage.removeBgAt = Date.now();
    sendMix(fakeImage);
}
...
const RABBITMQ_QUEUE_REMOVE_BG = process.env.RABBITMQ_QUEUE_REMOVE_BG || 'ai.remove.bg';
...
amqp.connect(RABBITMQ_CONNECTION_URI, {}, async (errorConnect, conn) => {
    ...
    await connection.createChannel(async (errorChannel, ch) => {
        ...
        await ch.assertQueue(RABBITMQ_QUEUE_REMOVE_BG, {}, (errorQueue) => {
            if (errorQueue) {
                console.error(errorQueue);
                process.exit(-1);
            }
            console.debug("Remove BG queue asserted");
        });

        channel = ch;
        // Назначаю слушателя очереди
        channel.consume(RABBITMQ_QUEUE_REMOVE_BG,  async (data) => {
            let notification = JSON.parse(data.content.toString());
            let fakeImage = JSON.parse(data.content.toString());

            await removeBg(fakeImage);
